...
function DetailDialog({ data, handleDialog }: Props) {
  ...
  const addBookmark = (selected: CardDTO) => {
    ...
    if (!getLocalStorage || getLocalStorage === null) {
      ...
      // 2. 해당 이미지가 이미 로컬스토리지의 bookmark라는 데이터에 저장되어 있을 경우
      if (getLocalStorage.findIndex((item:CardDTO)=> item.id === selected.id) > -1) {
        toast("해당 이미지는 이미 북마크 저장되어있습니다. 😊")
      } else {
  ...
  useEffect(() => {
    ...
    // 위에 만들어놓은 escKeyDownCloseDialog를 키 다운했을 때, 이벤트로 등록
    ...
    <div className={styles.container}>
      <div className={styles.container__dialog}>
        ...
        <div className={styles.container__dialog__body}>
          <img src={data.urls.regular} alt="상세이미지" className={styles.image} />
        </div>
        <div className={styles.container__dialog__footer}>
          <div className={styles.infoBox}>
            <div className={styles.infoBox__item}>
              <span className={styles.infoBox__item__label}>이미지크기</span>
            ...
            
          {
            data && data.tags && data.tags.length > 0 ? (
              ...
              <div className={styles.tagBox__tag}>No tags</div>
            )
          }
          </div>
        </div>
